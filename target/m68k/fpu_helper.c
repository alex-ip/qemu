...
{
    uint64_t quotient;
    int sign = extractFloatx80Sign(val0->d) ^ extractFloatx80Sign(val1->d);

    res->d = floatx80_modrem(val0->d, val1->d, true, &quotient, &env->fp_status);
...
void HELPER(frem)(CPUM68KState *env, FPReg *res, FPReg *val0, FPReg *val1)
{
    ...
    fp_quot.d = floatx80_div(val0->d, val1->d, &fp_status);
